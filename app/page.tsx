...
import { defaultRequirement, createBaseRequirement, updateRequirementNutrient, type Requirement } from "@/lib/requirements";
...
export default function HomePage() {
  ...
  function recoverProfilesFromSavedFormulas() {
    const recoveredProfiles = savedFormulas
      ...
      .map((profile) => normalizeRequirement(profile));
  ...
  function restoreProfilesBackup() {
    ...
    try {
      ...
      const restoredProfiles = parsed.map((profile) => normalizeRequirement(profile));
  ...
  async function importProfiles(file: File) {
    try {
      ...
      const importedProfiles = parsed.map((profile) => normalizeRequirement(profile));
