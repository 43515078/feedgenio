...
const nutrientLabels: Record<NutrientKey, string> = {
  energy: "Energía metabolizable",
  protein: "Proteína cruda",
  crudeFiber: "Fibra cruda",
  lysine: "Lisina",
  methionine: "Metionina",
  metCys: "Metionina + cistina",
  threonine: "Treonina",
...
function cloneRequirementProfile(requirement: Requirement): Requirement {
  return {
    ...requirement,
...
function createEmptyRequirement(name: string): Requirement {
  const base = cloneRequirementProfile(defaultRequirement);
...
function normalizeRequirement(item: Partial<Requirement>): Requirement {
  const base = cloneRequirementProfile(defaultRequirement);
  const storedNutrients = item.nutrients || {};

  return {
    ...base,
    ...item,
    name: String(item.name || base.name),
    species: String(item.species || base.species),
    nutrients: Object.fromEntries(
      nutrientKeys.map((key) => [
        key,
        {
...
export default function HomePage() {
  ...
  function updateRequirement(field: string, value: unknown) {
    const updatedProfiles = requirementProfiles.map((profile, index) => {
      ...
      if (field === "nutrients" && value && typeof value === "object") {
        return {
          ...profile,
          nutrients: value as Requirement["nutrients"]
        };
      }

      if (
        field === "derivedRequirements" &&
        value &&
        typeof value === "object"
      ) {
        return {
          ...profile,
          derivedRequirements: value as Requirement["derivedRequirements"]
        };
      }

      if (nutrientKeys.includes(field as NutrientKey)) {
        const nutrient = field as NutrientKey;
      ...
      if (field.endsWith("Max")) {
        const nutrient = field.slice(0, -3) as NutrientKey;
  ...
  function saveCurrentFormula(costing?: SavedCosting) {
    ...
    const newFormula: SavedFormula = {
      ...
      result,
      ingredientsSnapshot: ingredients.map((ingredient) => ({
        ...ingredient,
        species: { ...ingredient.species },
        limits: Object.fromEntries(
          speciesKeys.map((species) => [
            species,
            { ...ingredient.limits[species] }
          ])
