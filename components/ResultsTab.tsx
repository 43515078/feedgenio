...
onSaveFormula: (costing?: SavedCosting) => void;
...
if (row.min > 0 && minDifference <= row.min * 0.03) {
...
alerts.push({
level: "danger",
message: "${row.label} está por debajo del mínimo: obtenido ${round( row.obtained, row.decimals )}${row.suffix}, mínimo ${round(row.min, row.decimals)}${row.suffix}."
});
}
...
(item) => item.status === "below" || item.status === "above"
) || [];
...
function saveFormulaWithCosting() {
onSaveFormula({
productionCostPerKg,
bagCostPer50Kg,
marginPercent
...
<h2>📊 Resultados de formulación</h2>
    ...
    {!result ? (
      ...
      <>
        ...
        <button className="action" type="button" onClick={saveFormulaWithCosting}>
          💾 Guardar fórmula
        </button>
  ...
  {result?.feasible && (
    <section className="card" style={{ marginTop: 18 }}>
      ...
      nutrientLimitStatuses.length === 0 ? (
        <div className="note">
          No hay ingredientes principales pegados a límites ni nutrientes
          fuera de rango.
        </div>
      ) : (
        <>
          {ingredientLimitStatuses.length > 0 && (
            <>
              <h3>📌 Ingredientes principales pegados</h3>
          ...
          {nutrientLimitStatuses.length > 0 && (
            <>
              <h3 style={{ marginTop: 16 }}>🧪 Nutrientes fuera de rango</h3>
